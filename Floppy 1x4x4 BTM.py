class Floppy:
    ...
    def move(self, move):
        ...
        if move == "F" or move == "Fw":
            ...
            for i in (0, 4, 8, 12):
                self.U[i], self.D[i] = self.D[i], self.U[i]
        if move == "Fw" or move == "f" or move == "S":
            self.R[1], self.L[2] = self.L[2], self.R[1]
            self.U[8:12], self.D[4:8] = self.D[4:8][::-1], self.U[8:12][::-1]
        elif move == "Rw" or move == "r" or move == "M":
            self.F[2], self.B[1] = self.B[1], self.F[2]
            for i in (2, 6, 10, 14):
                self.U[i], self.D[i] = self.D[i], self.U[i]
        if move == "b" or move == "S":
            self.R[2], self.L[1] = self.L[1], self.R[2]
            self.U[4:8], self.D[8:12] = self.D[8:12][::-1], self.U[4:8][::-1]
        elif move == "l" or move == "M":
            self.F[1], self.B[2] = self.B[2], self.F[1]
            for i in (1, 5, 9, 13):
                self.U[i], self.D[i] = self.D[i], self.U[i]
...
moves = ["F", "B", "R", "L", "Fw", "Rw", "f", "r", "l", "b", "M", "S"]
tabla = {Floppy()}
frontera = [Floppy()]
depth = 0
